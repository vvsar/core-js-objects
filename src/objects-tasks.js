...
function mergeObjects(objects) {
  ...
  const arr1 = objects.map((el) => Object.entries(el));
  const arr2 = arr1.flat();
  const keys = Array.from(new Set(arr2.map((el) => el[0])));
  const result = {};
...
function makeWord(lettersObject) {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    for (let j = 0; j < entries[i][1].length; j += 1) {
      arr.push([entries[i][1][j], entries[i][0]]);
    }
  }
...
function sortCitiesArray(arr) {
  ...
  arr.sort((c, d) => c.country.localeCompare(d.country));
  return arr;
}
...
function group(array, keySelector, valueSelector) {
  const set = new Set();
  array.forEach((el) => set.add(keySelector(el)));
  const map = new Map();
  set.forEach((el) => map.set(el, []));
  array.forEach((el) => map.get(keySelector(el)).push(valueSelector(el)));
  return map;
}
